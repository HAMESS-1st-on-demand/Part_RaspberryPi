...
int main() {
    ...
    // UART 포트 열기
    uart_fd = open("/dev/ttyS0", O_RDWR | O_NOCTTY); // | O_NDELAY); // O_RDWR : 읽고 쓰기, O_NOCTTY : 키보드 중단 신호 영향, O_NDELAY : 수신 대기 차단 
    if (uart_fd == -1) {
    ...
    options.c_cflag = B9600 | CS5 | CLOCAL | CREAD; // Baud rate 설정: 9600, 5-bit, 지역 연결, 데이터 수신 허용 (bit 5 ~ 8)
    ...
    // 데이터 송신
    char tx_buffer[256];
    strcpy(tx_buffer, "Hello from UART!\n");
