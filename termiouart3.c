...
int main() {
    ...
    for (int i = 0; i < MAX_TRIES; i++) {
        ...
        long elapsed_time = get_microseconds() - start_time; // time elapsed till now in microseconds
        if (rv == -1) {
            perror("select()");
        } else if (rv == 0) {
            printf("Timeout occurred! No data after %ld microseconds.\n", elapsed_time);
            fprintf(logFile, "Timeout occurred! No data after %ld microseconds.\n", elapsed_time);
        } else {
